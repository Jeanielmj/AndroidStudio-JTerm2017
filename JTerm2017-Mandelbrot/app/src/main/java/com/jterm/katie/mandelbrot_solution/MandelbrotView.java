...
public class MandelbrotView extends View {
    ...
    private double mXRange = mXMax - mXMin;
    private double mYMin = Y_MIN;
    private double mYMax = Y_MAX;
    private double mYRange = mYMax - mYMin;
    ...
    /*
     * loop through all pixels from i = 0 > height & j = 1> weight
     *
     * pixel coord to
     * coord(0,0) = (-2.5, 1.75)
     *
    ...
    private int getMandelbrotColor(double mX, double mY) {
        ...
        // check divergence
        while (iter < mMaxDepth && (zx * zx) + (zy * zy) < (2 * 2) ){
            double tx = zx;
            zx = (zx * zx) - (zy * zy) + mX;
            zy = 2 * tx * zy + mY;
            iter++;
